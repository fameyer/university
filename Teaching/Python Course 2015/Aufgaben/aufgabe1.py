...
def main():
    '''Anlegen eines Gleichungssystems, Lösen und Ausgabe des Ergebnisses'''

    # lege Matrix und Vektor an
    A = np.array([[2, -1, 5, 7], \
                  [-1, 1, 1, 4], \
                  [1, 2, -3, -2], \
                  [3, -1, 1, 0]], dtype=float)
    b = np.array([43, 20, -12, 4], dtype=float)

    # bestimme Lösung und Residuum
    x = gauss_algorithmus(A, b)
    res = np.dot(A, x) - b

    # gebe Ergebnisse aus
    print('Lösung mit Gauß-Algorithmus: x = {}'.format(x))
    print('Probe: ||Ax-b|| = {}'.format(np.sqrt(sum(res**2))))
